...
# Timeframe para sa Chart at Current TF Analysis
...
if HAS_LIVE_DATA:
    try:
        ...
        # 2. Current TF Data Computation
        df_curr = yf.download(yf_symbol, period="7d", interval=tf_map_yf, progress=False)
...
if latest_close <= htf_ob_high:
    ...
    st.warning(f"💡 **HTF Recommendation:** Ang presyo ay umabot na sa Daily FVG / Resistance zone (**${htf_fvg_low:,.2f} - ${htf_fvg_high:,.2f}**). Mag-ingat sa pag-akyat; asahan ang posibleng **HTF Correction o Short**. dak")
else:
